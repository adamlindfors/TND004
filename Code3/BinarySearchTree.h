...
class BinarySearchTree
{
  ...
  private:
    ...
    BinaryNode* remove( const Comparable & x, BinaryNode *t )
    {
        ...
        {
            t->element = findMin( t->right )->element; // Replace node with next smallest			
            t->right = remove( t->element, t->right ); // Remove next smallest instead 
        }
        else
        {
			...
			if (t->left != nullptr)
			{
				...
				if (oldNode->element < oldNode->parent->element)
				{
					t->parent->left = t;
				}
				else
